...
class MLM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Conditionally use FlowLoss or DiffLoss based on config
        if use_flow:
            from flow_loss import FlowLoss
            self.diffloss = FlowLoss(
                target_channels=vae_latent_channels,
                z_channels=self.hidden_dim,
                depth=diffhead['params'].get('depth', 3),
                width=diffhead['params'].get('width', 1024),
                num_sampling_steps=diffhead['params'].get('num_sampling_steps', 50),
                target_size=vae_latent_size,
        ...
        self.use_rk4_sampling = use_rk4_sampling
        self.sampling_steps = sampling_steps
        print(f"[DEBUG] MLM initialized with use_rk4_sampling={use_rk4_sampling}, sampling_steps={sampling_steps}")
    ...
    def sample_tokens(self, outputs, device, temperature=1.0, cfg=1.0, vae_mean_std_inv=None):
        print(f"[DEBUG] sample_tokens called: use_rk4={self.use_rk4_sampling}, steps={self.sampling_steps}, diffloss_type={type(self.diffloss).__name__}")
        ...
        # Use RK4 or Euler based on config (num_steps only for FlowLoss)
        if hasattr(self.diffloss, 'sample_rk4'):
            # FlowLoss - supports num_steps and RK4
            if self.use_rk4_sampling:
                sampled_token_latents = self.diffloss.sample_rk4(output_to_sample, temperature, cfg, num_steps=self.sampling_steps)
            else:
                sampled_token_latents = self.diffloss.sample(output_to_sample, temperature, cfg, num_steps=self.sampling_steps)
        else:
            # DiffLoss - doesn't support num_steps
            print("[DEBUG] Using DiffLoss sampling")
            sampled_token_latents = self.diffloss.sample(output_to_sample, temperature, cfg)
        if cfg > 1.0:
