...
def resume_config(cfg: OmegaConf):
    ...
    if cfg.TRAIN.RESUME:
        ...
        if os.path.exists(resume):
            ...
            # Wandb (optional - may not exist if training didn't use wandb)
            wandb_latest_run = pjoin(resume, "wandb", "latest-run")
            if os.path.exists(wandb_latest_run):
                wandb_files = os.listdir(wandb_latest_run)
