...
class BaseModel(LightningModule):
    def __init__(self, *args, **kwargs):
        ...
        # Best metrics tracking (simple dict, only used on rank 0 for logging)
        self._best_metrics = {
            'fid': 1000.0,
            'r1': 0.0,
            'r2': 0.0,
            'r3': 0.0,
            'matching_score': 1000.0,
        }
    ...
    def on_train_epoch_end(self):
        ...
        if not self.trainer.sanity_checking:
            ...
            # Log to wandb directly (only on rank 0)
    ...
    def on_validation_epoch_end(self):
        ...
        # Write to log only if not sanity check
        if not self.trainer.sanity_checking:
            # Track and log best metrics (only on rank 0)
            # Ensure _best_metrics exists (in case model was recreated by DDP)
            ...
            if 'Metrics/FID' in dico:
                if dico['Metrics/FID'] < self._best_metrics['fid']:
                    self._best_metrics['fid'] = dico['Metrics/FID']
                dico['Metrics_Best/FID'] = self._best_metrics['fid']

            if 'Metrics/R_precision_top_1' in dico:
                if dico['Metrics/R_precision_top_1'] > self._best_metrics['r1']:
                    self._best_metrics['r1'] = dico['Metrics/R_precision_top_1']
                dico['Metrics_Best/R_precision_top_1'] = self._best_metrics['r1']

            if 'Metrics/R_precision_top_2' in dico:
                if dico['Metrics/R_precision_top_2'] > self._best_metrics['r2']:
                    self._best_metrics['r2'] = dico['Metrics/R_precision_top_2']
                dico['Metrics_Best/R_precision_top_2'] = self._best_metrics['r2']

            if 'Metrics/R_precision_top_3' in dico:
                if dico['Metrics/R_precision_top_3'] > self._best_metrics['r3']:
                    self._best_metrics['r3'] = dico['Metrics/R_precision_top_3']
                dico['Metrics_Best/R_precision_top_3'] = self._best_metrics['r3']

            if 'Metrics/Matching_score' in dico:
                if dico['Metrics/Matching_score'] < self._best_metrics['matching_score']:
                    self._best_metrics['matching_score'] = dico['Metrics/Matching_score']
                dico['Metrics_Best/Matching_score'] = self._best_metrics['matching_score']
    ...
    def step_log_dict(self):
        # Use epoch 0 for initial validation, otherwise current_epoch + 1
        if getattr(self, 'initial_validation', False):
            epoch = 0
            step = 0
        else:
            epoch = self.trainer.current_epoch + 1
            step = float(epoch) * len(self.datamodule.train_dataset)
        return {
            "epoch": float(epoch),
            "step": step,
    ...
    def metrics_log_dict(self):
        ...
        for metric in metrics_dicts:
            ...
            for metric_name, value in metrics_dict.items():
                ...
                # Separate gt metrics into their own section
                if metric_name.startswith('gt_'):
                    metrics_log_dict[f"Metrics_GT/{metric_name[3:]}"] = value
                else:
                    metrics_log_dict[f"Metrics/{metric_name}"] = value
